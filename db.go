...
type DB interface {
	...
	// the value,
	GetOrFill(key string, fill func() interface{}, tags ...string) (val interface{}, ok bool)
...
func (c *db) GetOrFill(key string, fill func() interface{}, tags ...string) (val interface{}, ok bool) {
	val, ok = c.Get(key)
	if ok {
		return
	}
...
func (c *transaction) Tag(key string, tags ...string) {
	if _, ok := c.additions.keyToTags[key]; !ok {
		c.additions.keyToTags[key], ok = c.origin.keyToTags[key]
		if !ok {
